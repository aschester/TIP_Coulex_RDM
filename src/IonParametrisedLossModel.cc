...
#include "G4IonStoppingData.hh"        // geant default for inclusion of ICRP 73
...
IonParametrisedLossModel::IonParametrisedLossModel(
    ...
    cutEnergies(0) 
{
  ...
  // energies in MeV/A
  lowerEnergyEdgeIntegr = 0.025 * MeV; // default
...
G4double IonParametrisedLossModel::MinEnergyCut(
                                       const G4ParticleDefinition*,
...
G4double IonParametrisedLossModel::MaxSecondaryEnergy(
...
G4double IonParametrisedLossModel::GetChargeSquareRatio(
                             const G4ParticleDefinition* particle,
...
G4double IonParametrisedLossModel::GetParticleCharge(
                             const G4ParticleDefinition* particle,
			     const G4Material* material,
                             G4double kineticEnergy) {   // Kinetic energy 
...
void IonParametrisedLossModel::Initialise(
                                       const G4ParticleDefinition* particle,
  ...
  size_t nmbCouples = coupleTable -> GetTableSize();
    ...
    G4cout << "IonParametrisedLossModel::Initialise():"
...
G4double IonParametrisedLossModel::ComputeCrossSectionPerAtom(
                                   const G4ParticleDefinition* particle,
                                   G4double kineticEnergy,
  ...
  G4cout << "########################################################"
         << G4endl
         << "# IonParametrisedLossModel::ComputeCrossSectionPerAtom"
         << G4endl
...
G4double IonParametrisedLossModel::CrossSectionPerVolume(
				   const G4Material* material,
                                   const G4ParticleDefinition* particle,
                                   G4double kineticEnergy,
                                   G4double cutEnergy,
...
G4double IonParametrisedLossModel::ComputeDEDXPerVolume(
                                      const G4Material* material,
			              const G4ParticleDefinition* particle,
				      G4double kineticEnergy,
				      G4double cutEnergy) {
...
void IonParametrisedLossModel::PrintDEDXTable(
  ...
  G4cout << "# Custom dE/dx model implemented by ASC"<< G4endl;
...
void IonParametrisedLossModel::PrintDEDXTableHandlers(
                   const G4ParticleDefinition* particle,  // Projectile (ion) 
                   const G4Material* material,  // Absorber material
                   G4double lowerBoundary,      // Minimum energy per nucleon
                   G4double upperBoundary,      // Maximum energy per nucleon
...
void IonParametrisedLossModel::SampleSecondaries(
  ...
  do {
    ...
    if(g > 1.0) {
        G4cout << "IonParametrisedLossModel::SampleSecondary Warning: "
               << "Majorant 1.0 < "
...
void IonParametrisedLossModel::UpdateRangeCache(
                     const G4ParticleDefinition* particle,
...
void IonParametrisedLossModel::UpdateDEDXCache(
                     const G4ParticleDefinition* particle,
                     const G4Material* material,
                     G4double cutEnergy) {
...
void IonParametrisedLossModel::CorrectionsAlongStep(
                             const G4MaterialCutsCouple* couple,
  ...
  G4cout << "########################################################"
         << G4endl
         << "# IonParametrisedLossModel::CorrectionsAlongStep"
         << G4endl
         << "# cut(MeV) = " << cutEnergy/MeV 
         << G4endl;
...
void IonParametrisedLossModel::BuildRangeVector(
                     const G4ParticleDefinition* particle,
                     const G4MaterialCutsCouple* matCutsCouple) {
...
#ifdef PRINT_DEBUG_DETAILS
      G4cout << "IonParametrisedLossModel::BuildRangeVector() bin = " 
             << i <<", E = "
             << energy / MeV << " MeV, R = " 
             << range / mm << " mm"
             << G4endl;     
...
G4double IonParametrisedLossModel::ComputeLossForStep(
                     const G4MaterialCutsCouple* matCutsCouple,
                     const G4ParticleDefinition* particle,
                     G4double kineticEnergy,
                     G4double stepLength) {
...
#ifdef PRINT_DEBUG
     G4cout << "IonParametrisedLossModel::ComputeLossForStep() range = " 
...
G4bool IonParametrisedLossModel::AddDEDXTable(
                                const G4String& name,
                                G4VIonDEDXTable* table, 
                                G4VIonDEDXScalingAlgorithm* algorithm) {

  if(table == 0) {
     G4cerr << "IonParametrisedLossModel::AddDEDXTable() Cannot "
...
G4bool IonParametrisedLossModel::RemoveDEDXTable(
				 const G4String& name) {
...
void IonParametrisedLossModel::DeactivateICRU73Scaling() {

  RemoveDEDXTable("ICRU73");
  AddDEDXTable("ICRU73", new IonStoppingData("ion_stopping_data/icru73"));
}
