...
IonStoppingData::IonStoppingData(const G4String& leDirectory) :
  subDir( leDirectory ) {
...
IonStoppingData::~IonStoppingData() {
  ClearTable();
}

// #########################################################################

G4bool IonStoppingData::IsApplicable(
...
G4PhysicsVector* IonStoppingData::GetPhysicsVector(
         G4int atomicNumberIon,        // Atomic number of ion
         G4int atomicNumberElem        // Atomic number of elemental material
				    ) {
...
G4double IonStoppingData::GetDEDX(
...
G4bool IonStoppingData::AddPhysicsVector(
...
#ifdef G4VERBOSE
     G4cerr << "IonStoppingData::AddPhysicsVector() Error: Pointer to vector"
            << " is null-pointer."
            << G4endl;
#endif
...
#ifdef G4VERBOSE
     G4cerr << "IonStoppingData::AddPhysicsVector() Error: "
...
G4bool IonStoppingData::RemovePhysicsVector(
	G4int atomicNumberIon,         // Atomic number of ion
        const G4String& matIdentifier  // Name or chemical formula of material
				      ) {
...
#ifdef G4VERBOSE
     G4cerr << "IonStoppingData::RemovePhysicsVector() Warning: "
...
G4bool IonStoppingData::BuildPhysicsVector(
	G4int atomicNumberIon,          // Atomic number of ion
  ...
  // ASC - check to see if it opens the file
  // else
  //   {
  ...
  // ASC - check to see if it adds the physics vector
  else
    {
      G4cout << "Added physics vector from stopping data file: " << fileName << G4endl;
      // getc(stdin);
    }
...
void IonStoppingData::ClearTable() {
